    ...
    <div className="bg-white rounded-xl shadow-sm hover:shadow-xl transition-all duration-300 group border border-gray-100 hover:border-gray-200">
      <Link to={`/products/${product._id}`} className="block">
        ...
        <div className="relative overflow-hidden rounded-t-xl">
          <img
            src={product.images?.[0]?.url?.startsWith('http') ? product.images[0].url : `${import.meta.env.VITE_SERVER_URL || 'https://sportify-backend-theta.vercel.app'}/${(product.images?.[0]?.url || '').split('\\').join('/').replace(new RegExp('^/'), '')}`}
            alt={product.name}
